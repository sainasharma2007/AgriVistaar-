  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} disabled={loading} className="flex-1 border border-gray-300 text-gray-700 py-2.5 rounded-xl text-sm font-medium hover:bg-gray-50 disabled:opacity-50">{L.cancel}</button>
          <button onClick={onConfirm} disabled={loading} className="flex-1 bg-emerald-600 disabled:bg-gray-400 text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-emerald-700 transition-colors">
...
const FieldDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // ✅ No headers needed - token auto-attached by axios
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto space-y-4 pb-10">
            ...
            <div className="bg-white/70 rounded-xl p-3 space-y-2">
              {selectedScan.analysis.toLowerCase().includes("nitrogen") && (
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-gray-800">{language === "hi" ? "नाइट्रोजन स्प्रे करें" : "Apply Nitrogen spray"}</p>
                    <p className="text-xs text-gray-500">{language === "hi" ? "यूरिया या DAP 3 दिनों में।" : "Urea or DAP within 3 days."}</p>
                  </div>
                </div>
              )}
              {selectedScan.analysis.toLowerCase().includes("pest") && (
                <div className="flex items-start gap-2">
                  <span className="text-base">🐛</span>
                  <div>
                    <p className="text-xs font-semibold text-gray-800">{language === "hi" ? "कीटनाशक लगाएं" : "Apply pesticide"}</p>
                    <p className="text-xs text-gray-500">{language === "hi" ? "प्रभावित हिस्से पर।" : "Target affected area."}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <ul className="divide-y divide-gray-50">
            {scans.map((scan, idx) => {
              ...
              return (
                <li key={scan._id} className={`px-4 py-3 transition-colors ${isDeleting ? "opacity-40 pointer-events-none" : ""} ${isSelected ? "bg-emerald-50" : "hover:bg-gray-50"}`}>
                  <div className="flex items-center gap-2">
                    <div className="flex-1 flex items-center gap-2 cursor-pointer min-w-0" onClick={() => setSelectedScan(scan)}>
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-gray-900">{language === "hi" ? `स्कैन ${scans.length - idx}` : `Scan ${scans.length - idx}`}</p>
      ...
      <div className="flex gap-3">
        <button onClick={() => navigate(`/request-scan?field=${field._id}`)}
